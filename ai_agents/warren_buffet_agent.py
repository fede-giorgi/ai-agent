...
from tools.get_metrics import get_financial_metrics
from tools.get_financials import get_market_cap
from tools.get_financial_line_items import search_line_items
...
def warren_buffett_agent(state: dict):
    """
    The main agent function that orchestrates the entire stock analysis process
    from data fetching to generating a final signal.
    """
    data = state["data"]
...
def estimate_maintenance_capex(financial_line_items: list) -> float:
    """
    A helper function to estimate the capital expenditures required to maintain current operations.
    """
    if not financial_line_items:
        return 0
...
def calculate_intrinsic_value(financial_line_items: list) -> dict[str, any]:
    """
    Estimates the company's intrinsic value using a multi-stage Discounted Cash Flow (DCF) model based on owner earnings.
    """
...
def generate_buffett_output(
    ...
    """
    Packages all the analysis and uses an LLM to generate the final investment signal, confidence, and reasoning.
    """
